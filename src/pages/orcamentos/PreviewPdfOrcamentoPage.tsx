...
export default function PreviewPdfOrcamentoPage() {
  ...
  // RN-NOVA-2 — preview real via microsserviço (GET /orcamentos/{id}/preview-html), não mais
  // reimplementação local do layout: o botão de download só habilita quando esta chamada
  ...
  const carregarPreview = useCallback(async () => {
    ...
    await previewRetry.executar(async () => {
      const conteudo = await orcamentoService.buscarPreviewHtml(id)
      setHtml(conteudo)
    }, 'Não foi possível carregar o preview do orçamento.')
  ...
  // O botão principal fica desabilitado durante o cooldown (disabled abaixo) — o guard aqui é
  // defesa extra contra contornar o bloqueio da RN-NOVA-3 clicando de novo antes do re-render.
