...
export default function PreviewMultaPage() {
  ...
  // Épico #248 (V0.8.1) — preview via microsserviço (GET .../pdf-multa/preview-html), mesmo
  ...
  const carregarPreview = useCallback(async () => {
    ...
    await previewRetry.executar(async () => {
      const conteudo = await orcamentoService.buscarPreviewHtmlPdfMulta(id)
      setHtml(conteudo)
    }, 'Não foi possível carregar o preview do PDF de multa.')
  ...
  const handleDownloadPdf = () => {
    ...
    downloadRetry.executar(async () => {
      const blob = await orcamentoService.baixarPdfMulta(id)
      dispararDownloadBlob(blob, `multa-${orcamento?.numero || id}.pdf`)
    }, 'Erro ao baixar PDF de multa.')
  }
