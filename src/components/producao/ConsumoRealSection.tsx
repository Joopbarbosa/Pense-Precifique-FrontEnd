...
export default function ConsumoRealSection({ insumosConsumidos, valores, onChange, titulo }: Props) {
  ...
  return (
    <div data-testid={titulo ? `consumo-real-section-${titulo}` : 'consumo-real-section'} className="flex flex-col gap-2.5">
      {titulo && (
      ...
      {insumosConsumidos.map((item, i) => {
        ...
        // Escopado por `titulo` porque AgruparProducoesModal renderiza várias instâncias deste
        // componente ao mesmo tempo (uma por produção agrupada) — sem o prefixo, o mesmo insumo
        // consumido em mais de uma produção geraria data-testid duplicado no DOM.
        const chaveOuIndice = chave || i
        const testIdBase = titulo ? `${titulo}-${chaveOuIndice}` : `${chaveOuIndice}`
        return (
          <div key={chave || i} data-testid={`consumo-real-item-${testIdBase}`} className="flex items-center justify-between gap-3 rounded-[10px] border border-line bg-cream px-3.5 py-3">
