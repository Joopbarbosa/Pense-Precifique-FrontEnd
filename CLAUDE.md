...
- **`ModalShell` é o padrão oficial de todo modal do sistema** (V0.6.1.1) — nunca reimplementar overlay/painel/botão-fechar inline. Renderiza `title` acima de `subtitle` (ordem corrigida em `434034c`, era invertida) e fecha via Escape. Débito conhecido, não migrado nesta leva: wizard de cancelamento com multa/estorno em `DetalheOrcamentoPage.tsx` e `CompraLoteModal`/`ImpactoLoteModal` em `ListaInsumosPage.tsx` ainda reimplementam modal inline.
