...
> **V0.8.2** — Lido automaticamente pelo Claude Code.
...
> Atualizado em: 2026-08-14 — #218 (V0.8, RN-NOVA-8/9/11) concluído: bloqueio/aviso de estoque em Novo Orçamento. `CriarOrcamentoPage.tsx` ganhou `verificarEstoqueAoAdicionar` (bloqueia adição via toast quando `permitirEstoqueNegativo=false` e estoque insuficiente, RN-NOVA-8), aviso inline em `ItemRow` + modal própria ao clicar "Criar orçamento" com link "Criar produção" para itens em `AVISO` (RN-NOVA-9), e um `useEffect` com debounce de 300ms que reconsulta `POST /orcamentos/simular-alertas` a cada mudança na lista de itens, para que o estoque exibido nunca fique preso ao snapshot da adição (RN-NOVA-11). Consome `SimulacaoEstoqueProdutoResponse` (DTO dedicado do backend, `orcamentoService.simularEstoque`). Sessão anterior foi interrompida por queda de energia por volta dos 20min sem persistir — investigação read-only confirmou que o código em disco, ainda não commitado, já estava completo e `npm run build` passava limpo; retomada desta sessão só precisou rodar a suíte, validar visualmente e documentar. Achado durante a rodada da suíte completa: `e2e/orcamento/simular-alertas.spec.ts` (pré-existente, não tocado pela sessão anterior) ainda esperava o campo antigo `nomeInsumo` — o backend do próprio #218 renomeou para `nomeProduto` num DTO dedicado, e esse teste tinha ficado pra trás; corrigido nesta sessão. 39/39 testes verdes em `e2e/orcamento/`. Validação visual manual via Playwright direto no app rodando (screenshots do bloqueio, aviso inline e modal de avanço).
> Atualizado em: 2026-08-14 — Retomada de fechamento V0.8: épico #89 (Microsserviço de PDF, `pense-precifique-pdf`) consolidado como arquitetura padrão de geração de documento — `PreviewPdfOrcamentoPage.tsx` (renomeada de `PreviewPdfPage.tsx`) consome `GET /orcamentos/{id}/preview-html` via `<iframe srcDoc>`, gate real de download (botão só habilita com preview OK), regressão de layout do documento (visual básico herdado da consolidação de #89) corrigida via Claude Design antes do fechamento (commit `66b18d7` do microsserviço). Padrão novo reutilizável: `useRetryCooldown`/`RetryCooldownModal` (10s de cooldown), usado nos 4 pontos de geração/download de PDF — ver "Aprendizados críticos". #217 (busca de item de catálogo server-side + listagem 8 itens) e #233 (botão "Criar produção" já existia desde `7a3a0df` — divergência era só de documentação, sem código a corrigir) fecham o pocket. Consolidação documental completa em `docs-pense-precifique` (`modulos/ORCAMENTO/`, `modulos/PDF/` novo).
> Atualizado em: 2026-08-30 — Retomada de fechamento V0.8.2: revisão completa do vínculo Orçamento↔Produção (#320) fechada de ponta a ponta (P-F004/P-F005). `ModalVincularProducao` (`components/orcamento/`, extraído para compartilhado entre `CriarOrcamentoPage`/`DetalheOrcamentoPage`) ganhou 3º modo (`modoCriarNova`, mini-formulário de criar produção nova já vinculada, `POST /orcamentos/{id}/criar-producao-vinculada`) além dos 2 existentes (lista/confirmar) — guarda local (`criandoNova`) contra double-submit, sem depender do estado do chamador. Card "Vinculado a produção" no Detalhe passou a consumir `orcamento.producoesVinculadas` direto de `GET /orcamentos/{id}` (nunca mais estado local perdido no reload). Demais fechamentos: mini-estorno de multa exibido na tela final (P-F006), edição de orçamento em Rascunho reaproveitando `CriarOrcamentoPage` em modo edição (P-F007), fluxo de duplicar orçamento (P-F008). **Achado de prioridade Alta para V0.8.3, não corrigido nesta versão:** `ItemSearch` (`CriarOrcamentoPage.tsx`) nunca foi atualizado para consumir a paginação real que `GET /orcamentos/itens-catalogo` passou a devolver desde #353 (V0.8.2) — catálogos com mais de 8 itens têm itens 9+ permanentemente invisíveis na busca, confirmado por teste E2E automatizado (P-T003). **Desvincular produção (RN-ORC-VINC-03) não tem nenhum consumidor de UI** — endpoint pronto e testado no backend, sem botão na tela (achado reconfirmado 3x na mesma versão, ver `CSV_FINAL_V0.8.3_V0.8.4.csv`). Testes E2E novos: `e2e/orcamento/mini-estorno.spec.ts`, `editar-rascunho.spec.ts`, `duplicar.spec.ts`, `vinculo-producao.spec.ts` — os 2 últimos cenários de `vinculo-producao.spec.ts` (desvincular) rodam só via API, sem `page`, pelo motivo acima.
...
| Erro de geração/download assíncrono usa `useRetryCooldown` + `RetryCooldownModal`, nunca `alert()` | Padrão estabelecido em #89 (V0.8) para os 4 pontos de preview/download de PDF de Orçamento — cooldown de 10s, contagem regressiva, botão de origem também desabilitado durante o cooldown (não só o "Tentar novamente" da modal). Reaproveitar para qualquer novo fluxo de geração assíncrona sujeita a falha temporária de serviço externo, em vez de reimplementar. |
| Axios com `responseType: 'blob'`/`'text'` não faz parse automático do corpo de erro | `err.response.data` chega como `Blob`/string crua, não JSON — `extractApiError` (espera `.message`) cai no fallback genérico silenciosamente. `normalizarErroBlob` (`src/utils/apiError.ts`) lê o Blob como texto e faz `JSON.parse` antes do catch. Endpoints que devolvem texto de sucesso (HTML) E erro (JSON) no mesmo caminho: não declarar `responseType` (deixar o Axios tentar `JSON.parse`, cair pro texto cru se falhar). |
