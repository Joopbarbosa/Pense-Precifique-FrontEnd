/* recibo-sinal-app.jsx — Tela 18 · Preview do Recibo do Sinal · Pense & Precifique */
const { useState, useEffect } = React;

/* ─────────  LOGO / WORDMARK  ───────── */
function Logo({ size = 40 }) {
  return <img src={(window.__resources && window.__resources.logo) || "logo.png"} width={size} height={size} alt="Pense & Precifique"
...
const I = {
  ...
  checkCircle:(p)=><svg viewBox="0 0 24 24" width="22" height="22" {...sw} {...p}><circle cx="12" cy="12" r="8.5"/><path d="m8.3 12.2 2.5 2.5 4.9-5" strokeWidth="2"/></svg>,
  check:(p)=><svg viewBox="0 0 24 24" width="14" height="14" {...sw} {...p}><path d="m5 12.5 4.2 4.2L19 7" strokeWidth="2.2"/></svg>,
  pix:(p)=><svg viewBox="0 0 24 24" width="14" height="14" {...sw} {...p}><path d="M12 3.8 8.6 7.2a2 2 0 0 0 0 2.8L12 13.4l3.4-3.4a2 2 0 0 0 0-2.8L12 3.8ZM12 20.2 8.6 16.8a2 2 0 0 1 0-2.8L12 10.6l3.4 3.4a2 2 0 0 1 0 2.8L12 20.2ZM3.8 12 7.2 8.6a2 2 0 0 1 2.8 0M16 8.6 19.4 12l-3.4 3.4"/></svg>,
  bag:(p)=><svg viewBox="0 0 24 24" width="15" height="15" {...sw} {...p}><path d="M6 8.5h12l-1 11.5a1 1 0 0 1-1 .9H8a1 1 0 0 1-1-.9L6 8.5Z"/><path d="M9 8.5V7a3 3 0 0 1 6 0v1.5"/></svg>,
  sparkles:(p)=><svg viewBox="0 0 24 24" width="16" height="16" {...sw} {...p}><path d="M12 3.5 13.6 9 19 10.6 13.6 12.2 12 17.7 10.4 12.2 5 10.6 10.4 9 12 3.5ZM18.5 15.5l.7 2.3 2.3.7-2.3.7-.7 2.3-.7-2.3-2.3-.7 2.3-.7.7-2.3Z"/></svg>,
...
function DocLogo({ size = 56 }) {
  return <img src={(window.__resources && window.__resources.logo) || "logo.png"} width={size} height={size} alt="" style={{ objectFit:'contain', flexShrink:0 }} />;
}

/* ─────────  DADOS  ───────── */
const ITENS = [
  { nome:'Kit Convite Casamento', custom:'Laminação fosca', qtd:3 },
  { nome:'Etiqueta personalizada', custom:'—', qtd:10 },
];
const VALOR_TOTAL = 183.60, VALOR_SINAL = 91.80, RESTANTE = 91.80;
    ...
    <div className="a4" style={{ animation:'fadeUp .4s ease both' }}>
      ...
      <div style={{ marginTop:26, padding:'20px 22px', borderRadius:12, background:hexA(teal,0.07), border:`1px solid ${hexA(teal,0.22)}`, borderLeft:`4px solid ${teal}` }}>
        <div style={{ display:'flex', alignItems:'center', gap:13 }}>
          <span style={{ flexShrink:0, display:'grid', placeItems:'center', width:44, height:44, borderRadius:12, background:'#fff', color:teal, border:`1px solid ${hexA(teal,0.25)}` }}><I.checkCircle /></span>
          <div>
            <h1 style={{ margin:0, fontSize:18, fontWeight:700, color:'var(--teal-deep)', letterSpacing:'-0.01em', lineHeight:1.2, textWrap:'balance' }}>RECIBO DE PAGAMENTO — ENTRADA</h1>
      ...
      {/* confirmação do recebimento — destaque */}
      <div style={{ marginTop:24, borderRadius:14, overflow:'hidden', border:`1.5px solid ${hexA(teal,0.32)}`, background:hexA(teal,0.05) }}>
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'14px 18px', background:`linear-gradient(135deg, ${hexA(teal,0.14)}, ${hexA(teal,0.04)})`, borderBottom:`1px solid ${hexA(teal,0.2)}` }}>
          <span style={{ flexShrink:0, display:'grid', placeItems:'center', width:30, height:30, borderRadius:9, background:'#fff', color:teal, boxShadow:`0 3px 9px -3px ${hexA(teal,0.4)}` }}><I.check /></span>
          <span style={{ fontSize:15, fontWeight:700, color:'var(--teal-deep)', letterSpacing:'-0.005em' }}>Entrada recebida com sucesso</span>
        </div>
        <div style={{ display:'flex', flexWrap:'wrap', alignItems:'flex-end', gap:'18px 24px', padding:'18px' }}>
          <div style={{ flex:'1 1 180px' }}>
            <div style={{ fontSize:11, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.04em', color:'var(--teal-deep)' }}>Valor recebido</div>
            <div style={{ fontSize:34, fontWeight:700, color:teal, letterSpacing:'-0.02em', lineHeight:1.05, marginTop:3, fontVariantNumeric:'tabular-nums' }}>{BRL(VALOR_SINAL)}</div>
          </div>
          <div style={{ display:'flex', gap:'18px 28px', flexWrap:'wrap' }}>
            <div>
              ...
              <div style={{ fontSize:14, fontWeight:600, color:'var(--ink)', marginTop:3, fontVariantNumeric:'tabular-nums' }}>05/06/2026</div>
            </div>
            <div>
              <div style={{ fontSize:10.5, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.04em', color:'#9A968E' }}>Forma de pagamento</div>
              <div style={{ display:'inline-flex', alignItems:'center', gap:6, fontSize:14, fontWeight:600, color:'var(--ink)', marginTop:3 }}><span style={{ color:teal, display:'flex' }}><I.pix /></span>PIX</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop:26 }}>
        <SectionTitle teal={teal} icon={<I.bag />}>Detalhes do pedido</SectionTitle>
        <table className="a4-table" style={{ marginTop:12 }}>
          <thead>
            <tr><th>Item</th><th>Customização</th><th className="num">Qtd</th></tr>
          </thead>
          <tbody>
            {ITENS.map((it,i)=>(
              <tr key={i}>
        ...
        <div style={{ display:'flex', justifyContent:'flex-end', marginTop:18 }}>
          <div style={{ width:'min(340px, 100%)' }}>
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:13.5, color:'#5C594F', padding:'8px 12px' }}>
              <span>Valor total do pedido</span><span style={{ fontVariantNumeric:'tabular-nums', fontWeight:600, color:'var(--ink)' }}>{BRL(VALOR_TOTAL)}</span>
            </div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', fontSize:13.5, color:'#5C594F', padding:'10px 12px', borderRadius:9, background:hexA(teal,0.07), border:`1px dashed ${hexA(teal,0.4)}`, margin:'2px 0' }}>
              <span style={{ display:'flex', alignItems:'center', gap:6, fontWeight:600, color:teal }}><I.check /> Entrada paga (50%)</span>
              <span style={{ fontWeight:700, color:teal, fontVariantNumeric:'tabular-nums' }}>{BRL(VALOR_SINAL)}</span>
            </div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', marginTop:6, padding:'13px 14px', borderRadius:10, background:'var(--orange-soft)', border:`1px solid ${hexA(orange,0.28)}` }}>
              <span style={{ fontSize:13.5, fontWeight:600, color:'var(--ink)', lineHeight:1.3 }}>Restante a pagar na entrega</span>
      ...
      {/* próximos passos */}
      <div style={{ marginTop:26 }}>
        <SectionTitle teal={teal} icon={<I.sparkles width="15" height="15" />}>Próximos passos</SectionTitle>
        <div style={{ marginTop:12, padding:'16px 18px', borderRadius:12, background:hexA(teal,0.05), border:`1px solid ${hexA(teal,0.2)}` }}>
          <p style={{ margin:0, fontSize:13.5, color:'#3A372F', lineHeight:1.65 }}>
            <strong style={{ fontWeight:700, color:'var(--teal-deep)' }}>Sua produção foi iniciada!</strong> O restante de <strong style={{ fontWeight:700, color:orange }}>{BRL(RESTANTE)}</strong> será cobrado na entrega do pedido. Prazo estimado: <strong style={{ fontWeight:700, color:'#5C594F' }}>10 dias úteis</strong>.
          </p>
        </div>
      </div>
          ...
          <div style={{ maxWidth:820, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, flexWrap:'wrap' }}>
            <div style={{ minWidth:0 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
                <h1 style={{ margin:0, fontSize:20, fontWeight:700, letterSpacing:'-0.02em', color:'var(--ink)' }}>Recibo do Sinal</h1>
                <span style={{ display:'inline-flex', alignItems:'center', gap:7, height:30, padding:'0 13px', borderRadius:999, background:'var(--orange-soft)', color:'#C2620E', fontSize:13, fontWeight:600 }}>
                  <span style={{ width:7, height:7, borderRadius:'50%', background:orange }} />Em Produção
                </span>
              </div>
            </div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
              ...
              <ActionBtn variant="teal" teal={teal} orange={orange}><I.download /> Baixar Recibo</ActionBtn>
            </div>
          </div>
        </div>
