/* recibo-pagamento-app.jsx — Preview do Recibo de Pagamento (Quitação Total) · Pense & Precifique */
...
const I = {
  ...
  seal:(p)=><svg viewBox="0 0 24 24" width="16" height="16" {...sw} {...p}><path d="M12 2.8l2.3 1.7 2.8-.3 1 2.6 2.4 1.5-.6 2.8 1.3 2.5-2 2-.2 2.8-2.7.8-1.7 2.3-2.6-1.1-2.6 1.1-1.7-2.3-2.7-.8-.2-2.8-2-2 1.3-2.5-.6-2.8L4.9 6.8l1-2.6 2.8.3L12 2.8Z"/><path d="m8.8 12 2.2 2.2 4.2-4.4" strokeWidth="1.9"/></svg>,
...
/* ─────────  DADOS  ───────── */
const GREEN = '#1B8A4B', GREEN_DEEP = '#136B39';
const ITENS = [
  { nome:'Kit Convite Casamento', custom:'Laminação fosca', qtd:3 },
  { nome:'Etiqueta personalizada', custom:'—', qtd:10 },
];
const VALOR_TOTAL = 183.60, VALOR_SINAL = 91.80, PAGO_AGORA = 91.80, SALDO = 0;
...
function Documento({ teal, orange }) {
  return (
    <div className="a4" style={{ animation:'fadeUp .4s ease both' }}>
      ...
      {/* título do recibo */}
      <div style={{ marginTop:26, padding:'20px 22px', borderRadius:12, background:hexA(GREEN,0.07), border:`1px solid ${hexA(GREEN,0.22)}`, borderLeft:`4px solid ${GREEN}` }}>
        <div style={{ display:'flex', alignItems:'center', gap:13 }}>
          <span style={{ flexShrink:0, display:'grid', placeItems:'center', width:44, height:44, borderRadius:12, background:'#fff', color:GREEN, border:`1px solid ${hexA(GREEN,0.25)}` }}><I.seal /></span>
          <div>
            <h1 style={{ margin:0, fontSize:18, fontWeight:700, color:GREEN_DEEP, letterSpacing:'-0.01em', lineHeight:1.2, textWrap:'balance' }}>RECIBO DE PAGAMENTO — QUITAÇÃO TOTAL</h1>
            <div style={{ display:'flex', flexWrap:'wrap', gap:'2px 16px', marginTop:5, fontSize:12.5, color:'#3F6B53' }}>
              ...
              <span>Data: <strong style={{ fontWeight:700 }}>09/06/2026</strong></span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop:24, borderRadius:14, overflow:'hidden', border:`1.5px solid ${hexA(GREEN,0.38)}`, background:hexA(GREEN,0.05) }}>
        <div style={{ display:'flex', alignItems:'center', gap:11, padding:'14px 18px', background:`linear-gradient(135deg, ${hexA(GREEN,0.15)}, ${hexA(GREEN,0.04)})`, borderBottom:`1px solid ${hexA(GREEN,0.22)}` }}>
          <span style={{ flexShrink:0, display:'grid', placeItems:'center', width:30, height:30, borderRadius:9, background:'#fff', color:GREEN, boxShadow:`0 3px 9px -3px ${hexA(GREEN,0.45)}` }}><I.check /></span>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', alignItems:'flex-end', gap:'18px 24px', padding:'18px' }}>
          <div style={{ flex:'1 1 220px' }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:8, fontSize:14, fontWeight:700, color:GREEN_DEEP, letterSpacing:'-0.005em' }}>
              <span style={{ flexShrink:0, display:'grid', placeItems:'center', width:20, height:20, borderRadius:'50%', background:GREEN, color:'#fff' }}><I.check width="12" height="12" /></span>
              Pagamento recebido em sua totalidade
            </div>
            <div style={{ fontSize:12.5, color:'#5C7A68', marginTop:6, lineHeight:1.5 }}>Não há valores pendentes para este orçamento.</div>
          </div>
          <div>
            <div style={{ fontSize:10.5, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.04em', color:'#9A968E' }}>Data do pagamento</div>
            <div style={{ fontSize:14, fontWeight:600, color:'var(--ink)', marginTop:3, fontVariantNumeric:'tabular-nums' }}>09/06/2026</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop:26 }}>
        <SectionTitle accent={GREEN} icon={<I.bag />}>Detalhes financeiros</SectionTitle>
        ...
        <div style={{ display:'flex', justifyContent:'flex-end', marginTop:18 }}>
          <div style={{ width:'min(380px, 100%)' }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', fontSize:13.5, padding:'8px 12px' }}>
              <span style={{ display:'flex', alignItems:'center', gap:6, color:'#5C594F' }}>Entrada já paga <span style={{ color:'#9A968E', fontSize:12 }}>em 05/06/2026</span></span>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', fontSize:13.5, padding:'8px 12px', borderBottom:'1px solid #F0EEE9' }}>
              ...
              <span style={{ fontWeight:600, color:'var(--ink)', fontVariantNumeric:'tabular-nums' }}>{BRL(PAGO_AGORA)}</span>
            </div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', fontSize:13.5, padding:'11px 12px', marginTop:4, borderRadius:9, background:hexA(GREEN,0.08), border:`1px dashed ${hexA(GREEN,0.4)}` }}>
              <span style={{ display:'flex', alignItems:'center', gap:6, fontWeight:600, color:GREEN_DEEP }}><I.check /> Saldo devedor</span>
              <span style={{ fontWeight:700, color:GREEN, fontVariantNumeric:'tabular-nums' }}>{BRL(SALDO)}</span>
            </div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', marginTop:8, padding:'14px 16px', borderRadius:10, background:'var(--orange-soft)', border:`1px solid ${hexA(orange,0.28)}` }}>
              <span style={{ fontSize:13.5, fontWeight:700, color:'var(--ink)', lineHeight:1.3 }}>Total quitado</span>
      ...
      <div style={{ marginTop:'auto', paddingTop:30 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, paddingTop:18, borderTop:'1px solid #F0EEE9', fontSize:11.5, color:'#9A968E', flexWrap:'wrap', lineHeight:1.6 }}>
          <span style={{ display:'flex', color:GREEN }}><I.seal width="15" height="15" /></span>
          Este recibo confirma a quitação total do pedido. Gerado pelo sistema
          <Wordmark teal={teal} size={11.5} />
          em <strong style={{ fontWeight:600, color:'#6B6860' }}>09/06/2026</strong>.
        </div>
      </div>
    </div>);
}

function SectionTitle({ accent, icon, children }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:9 }}>
      <span style={{ flexShrink:0, display:'grid', placeItems:'center', width:28, height:28, borderRadius:8, background:hexA(accent,0.1), color:accent }}>{icon}</span>
          ...
          <div style={{ maxWidth:820, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, flexWrap:'wrap' }}>
            <div style={{ minWidth:0 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
                <h1 style={{ margin:0, fontSize:20, fontWeight:700, letterSpacing:'-0.02em', color:'var(--ink)' }}>Recibo de Pagamento</h1>
